...
public class Sample2Models {
    public static Sample2Model sample() {
        ...
        Arrays.stream(wrapper.getFieldInfos()).map(FieldInfo::id)
                        .filter(id -> asList(id.tags()).contains(EMAIL))
                        .forEach(id -> wrapper.set(id, "account" + i.incrementAndGet() + "@some.name"));

        i.set(0);
        Arrays.stream(wrapper.getFieldInfos()).map(FieldInfo::id)
                        .filter(id -> asList(id.tags()).contains(ID))
                        .forEach(id -> wrapper.set(id, i.incrementAndGet()));

        i.set(0);
        Arrays.stream(wrapper.getFieldInfos()).map(FieldInfo::id)
                        .filter(id -> asList(id.tags()).contains(PASSWORD))
                        .forEach(id -> wrapper.set(id, "password_" + i.incrementAndGet()));

        i.set(0);
        Arrays.stream(wrapper.getFieldInfos()).map(FieldInfo::id)
                        .filter(id -> asList(id.tags()).contains(LOGIN))
